...
const resolveComponentStyleCodeId = (styleCode: string | StyleCode): string => {
  if (!styleCode) return ''
  if (typeof styleCode === 'string') return styleCode
  return styleCode.id || ''
}

const EditStyleCodeComboPage = () => {
  ...
  useEffect(() => {
    if (comboId) void loadCombo(comboId)
  }, [comboId])

  const loadCombo = async (id: string) => {
    try {
      setLoading(true)
      const data = await styleCodeComboService.get(id)
      setForm({
      ...
      const comps = (data.components || []).map((c) => ({
        styleCodeId: resolveComponentStyleCodeId(c.styleCode as string | StyleCode),
        quantity: Number(c.quantity) || 1,
      }))
      setComponents(comps.length > 0 ? comps : [{ styleCodeId: '', quantity: 1 }])
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await styleCodeComboService.update(comboId, {
      ...
      <div className="main-content">
        ...
        <div className="flex items-center justify-center py-16">
          <div className="text-center">
            <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-purple-600 mx-auto mb-4 opacity-60" />
            <p className="text-sm text-gray-600">Loading combo...</p>
          </div>
        </div>
      </div>
    )
    ...
    <div className="main-content">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold text-gray-900">Edit Combo</h1>
        </div>
        <Link
                  ...
                  <div key={index} className="grid grid-cols-1 md:grid-cols-[1fr_120px_40px] gap-2 items-end">
                    <div>
                      ...
                      >
                        ...
                        {row.styleCodeId &&
                          !styleCodeOptions.some((sc) => sc.id === row.styleCodeId) && (
                            <option value={row.styleCodeId}>{row.styleCodeId}</option>
                          )}
                      </select>
                    </div>
                    <div>
...
export default function EditStyleCodeComboPageWrapper() {
  return (
    <RequireCrudPermission path="Catalog.Style codes" action="update">
      <EditStyleCodeComboPage />
