...
import { getAttachmentDisplayName } from '@/shared/utils/hubFileDisplay';
...
/**
 * Read-only list of ticket/comment attachments with view + download links.
 */
  ...
  return (
    <ul className={compact ? 'mt-2 space-y-1' : 'grid gap-2 sm:grid-cols-2'}>
      {attachments.map((att, idx) => {
        const displayName = getAttachmentDisplayName(att.fileName);
        ...
        return (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-medium text-gray-700" title={displayName}>
                {displayName}
              </p>
            ...
            {att.url && (
              <div className="flex shrink-0 items-center gap-1">
                <a
                  ...
                  className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-gray-200 text-gray-600 transition hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-600"
