...
import TicketAssignControl from '../components/TicketAssignControl';
    ...
    return (
      <div className="mx-auto max-w-6xl p-4">
        <div className="flex flex-col items-center justify-center rounded-xl border border-gray-200 bg-white py-16 text-center shadow-sm">
          ...
          >
            <i className="ri-arrow-left-line" aria-hidden /> Back to hub
          </Link>
        </div>
      </div>
    );
        ...
        <header className="overflow-hidden rounded-xl border border-gray-300 bg-white shadow-sm">
              ...
              <div className="min-w-0 flex-1">
                <div className="mb-1.5 flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-[11px] font-medium text-gray-500">
                    Opened {formatHubDateTime(ticket.createdAt)}
                  </span>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span
                  className={`rounded-md px-2.5 py-1 text-[11px] font-semibold ${
                    !ticket.disposition || ticket.disposition === 'unset'
                      ? 'bg-gray-100 text-gray-600'
                      : 'bg-violet-50 text-violet-800'
                  }`}
                >
                  {formatDispositionLabel(ticket.disposition)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-4">
            ...
            <StatTile
              ...
              label="Assignee"
              value={
                isEmptyUserRef(ticket.assignedTo as { name?: string; email?: string })
                  ? 'Unassigned'
                  : userDisplayName(ticket.assignedTo as { name?: string; email?: string })
              }
            />
